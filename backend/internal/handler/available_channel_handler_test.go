...
func TestBuildPlatformSections_CustomGroupOnlyUsesCustomModels(t *testing.T) {
	customPrice := 2.5e-6
	ch := service.AvailableChannel{
		Name: "custom-channel",
		SupportedModels: []service.SupportedModel{
			...
			},
			{
				...
				Pricing:  &service.ChannelModelPricing{InputPrice: &customPrice},
			},
		},
	}
	visible := []userAvailableGroup{
		{ID: 9, Name: "custom", Platform: service.PlatformComposite},
	}

	sections := buildPlatformSections(ch, visible)

	require.Len(t, sections, 1)
	require.Equal(t, service.PlatformCustom, sections[0].Platform)
	require.Len(t, sections[0].Groups, 1)
	require.Equal(t, int64(9), sections[0].Groups[0].ID)
	require.Equal(t, service.PlatformComposite, sections[0].Groups[0].Platform)
	require.Len(t, sections[0].SupportedModels, 1)
	require.Equal(t, "vendor-chat-model", sections[0].SupportedModels[0].Name)
	require.Equal(t, service.PlatformCustom, sections[0].SupportedModels[0].Platform)
...
func TestBuildPlatformSections_CustomAndOrdinaryGroupsRemainIsolated(t *testing.T) {
	ch := service.AvailableChannel{
		SupportedModels: []service.SupportedModel{
			{Name: "claude-sonnet-4-6", Platform: service.PlatformAnthropic},
			{Name: "vendor-chat-model", Platform: service.PlatformCustom},
		},
	}
	visible := []userAvailableGroup{
		{ID: 1, Name: "anthropic-only", Platform: service.PlatformAnthropic},
	...
	require.Equal(t, service.PlatformCustom, sections[1].Platform)
	require.Len(t, sections[1].Groups, 1)
	require.Equal(t, int64(9), sections[1].Groups[0].ID)
}

func TestBuildPlatformSections_CustomWithoutModelsKeepsEmptyCustomSection(t *testing.T) {
	visible := []userAvailableGroup{
		{ID: 9, Name: "custom", Platform: service.PlatformComposite},
	}
