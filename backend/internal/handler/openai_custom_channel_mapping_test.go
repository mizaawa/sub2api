...
func newRestrictedCustomChannelMappingHandler(t *testing.T, accountMapping map[string]string) (*OpenAIGatewayHandler, *customChannelMappingHTTPUpstream) {
	t.Helper()
	gin.SetMode(gin.TestMode)
	...
	credentials := map[string]any{
		...
		"model_mapping": map[string]any{
			"kimi-k3": accountMapping["kimi-k3"],
		},
	}
	account := service.Account{
		ID:          7312,
	...
	channelRepo := &openAIWSUsageHandlerChannelRepoStub{
		channels: []service.Channel{{
			ID:                 7313,
			Name:               "custom-kimi-channel-restricted",
			Status:             service.StatusActive,
			GroupIDs:           []int64{groupID},
			RestrictModels:     true,
			...
			ModelPricing: []service.ChannelModelPricing{{
				Platform: service.PlatformCustom,
				Models:   []string{accountMapping["kimi-k3"]},
			}},
		}},
		groupPlatforms: map[int64]string{groupID: service.PlatformComposite},
	}
	...
	cfg.Security.URLAllowlist.Enabled = false
	cfg.Security.URLAllowlist.AllowInsecureHTTP = true
	upstream := &customChannelMappingHTTPUpstream{}
	billingCache := service.NewBillingCacheService(nil, nil, nil, nil, nil, nil, cfg, nil)
	concurrencyCache := &concurrencyCacheMock{
		acquireUserSlotFn:    func(context.Context, int64, int, string) (bool, error) { return true, nil },
		acquireAccountSlotFn: func(context.Context, int64, int, string) (bool, error) { return true, nil },
...
func TestCustomGroupChatAccountMappingRunsBeforeRestrictedChannelCheck(t *testing.T) {
	h, upstream := newRestrictedCustomChannelMappingHandler(t, map[string]string{
		"kimi-k3": "provider-model",
	})
