...
func TestNormalizeCodexAutomationBootstrapRejectsNonObjectInputItems(t *testing.T) {
	output := codexAutomationBootstrap("wiki", "never", automationBootstrapPrompt)
	for _, malformedItem := range []string{"null", `"not an input item"`, `42`, `[]`} {
		t.Run(malformedItem, func(t *testing.T) {
			body := []byte(`{"model":"gpt-5","input":[{"type":"function_call_output","namespace":"codex_app","name":"automation_update","output":` + mustJSON(t, output) + `},` + malformedItem + `]}`)
