...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Len(t, cache.tokens(), canonicalCount*2, "full rebuild and the following group event must each run fresh authority")
	_, reopenHeld := cache.lifecycleMutationLeaseStates()
	require.Len(t, reopenHeld, canonicalCount*2)
	for _, held := range reopenHeld {
		require.True(t, held)
	}
	require.Equal(t, (len(schedulerSnapshotPlatforms())+2)*3, accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	require.Equal(t, (len(schedulerSnapshotPlatforms())+2)*2+1, accounts.callCount())
...
func TestSchedulerFullRebuildPreservesGroupZeroActiveHistoricalAndInvalidRegistryBuckets(t *testing.T) {
	...
	registered := []SchedulerBucket{groupZeroHistorical, activeHistorical, activeForced, activeMixed, invalidHistorical}
	...
	require.NoError(t, svc.rebuildFullSnapshot(context.Background(), "test"))
	canonical := append(schedulerCanonicalBuckets(0), schedulerCanonicalBuckets(groupID)...)
	require.Equal(t, len(dedupeBuckets(append(canonical, registered...))), cache.captureAttemptCount())
	require.Equal(t, (len(schedulerSnapshotPlatforms())+2)*2+3, accounts.callCount())
	groups.mu.Lock()
	require.Equal(t, 1, groups.listCalls)
	groups.mu.Unlock()
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Zero(t, accounts.callCount())
	require.Zero(t, cache.totalSetAttempts())
	canonicalAndHistorical := len(schedulerCanonicalBuckets(0)) + 1
	require.Equal(t, canonicalAndHistorical, len(cache.retiredBuckets()))
	...
	require.Equal(t, canonicalAndHistorical*3, len(cache.retiredBuckets()))
...
func TestSchedulerFullRebuildActiveTombstoneLazyRecoveryDiscardsPartialCaptureTasks(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.Equal(t, len(schedulerCanonicalBuckets(0))+len(canonical)/2, capturesAtFirstDB)
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Equal(t, len(schedulerSnapshotPlatforms())+2+len(registered), accounts.callCount())
	require.Equal(t, len(schedulerSnapshotPlatforms())+2+len(registered), accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
	for _, bucket := range registered {
		_, published := cache.counts(bucket)
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	require.Equal(t, (len(schedulerSnapshotPlatforms())+2)*2, accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, (len(schedulerSnapshotPlatforms())+2)*4, accounts.callCount())
