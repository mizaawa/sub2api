...
func TestSettingService_UpdateSettings_PersistsDownstreamModelConsistencyBypass(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})

	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		DownstreamModelConsistencyBypassEnabled: true,
	})

	require.NoError(t, err)
	require.Equal(t, "true", repo.updates[SettingKeyDownstreamModelConsistencyBypassEnabled])
}
