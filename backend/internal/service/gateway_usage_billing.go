...
func postUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps) {
	...
	// Keep the caller's cost breakdown untouched for compatibility with
	// downstream observers, while all billing effects use the canonical amount.
	normalizedCost := *p.Cost
	normalizedCost.NormalizeMonetaryFields()
	cost := &normalizedCost
	...
	if p.shouldUpdateAccountQuota() {
		accountCost := quantizedAccountQuotaCost(cost.TotalCost, p.AccountRateMultiplier)
...
func applyUsageBilling(ctx context.Context, requestID string, usageLog *UsageLog, p *postUsageBillingParams, deps *billingDeps, repo UsageBillingRepository) (bool, *UsageBillingCommand, error) {
	...
	// copy above is used only for effects after that step, preserving retry
	// compatibility with fingerprints generated by older versions.
...
func quantizedAccountQuotaCost(totalCost, multiplier float64) float64 {
	return QuantizeUsageBillingAmount(totalCost * multiplier)
}
...
func notifyAccountQuota(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	accountCost := quantizedAccountQuotaCost(p.Cost.TotalCost, p.AccountRateMultiplier)
