...
// finalizeOperationalOrDegraded 负责走到最后一步的耗时状态判定。
...
func finalizeOperationalOrDegraded(res *CheckResult, latency time.Duration, latencyMs int) *CheckResult {
	if latency >= monitorFailedThreshold {
		res.Status = MonitorStatusFailed
		res.Message = truncateMessage(fmt.Sprintf("response exceeded latency limit: %dms", latencyMs))
		return res
	}
...
var providerAdapters = map[string]providerAdapter{
	...
	MonitorProviderCustom: providerCustomChatAdapter,
...
//nolint:gochecknoglobals // 适配器表是只读静态数据，初始化后不变更。
var providerCustomChatAdapter = newOpenAICompatibleChatAdapter(providerCustomPath)
...
func validateReplaceRequestBody(provider, apiMode string, body map[string]any) error {
	if provider != MonitorProviderOpenAI && provider != MonitorProviderGrok && provider != MonitorProviderCustom {
		return nil
	}
