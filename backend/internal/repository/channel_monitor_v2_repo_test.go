...
func TestChannelMonitorV2UsageSuccessExcludesCyberBillingRows(t *testing.T) {
	require.Contains(t, channelMonitorV2UsageMetricsSQL, "COALESCE(ul.request_type, 0) NOT IN (4, 6)")
	require.Contains(t, channelMonitorV2UsageMetricsSQL, "ul.actual_cost > 0")
...
func TestChannelMonitorV2AggregationDoesNotWritePerUserMetrics(t *testing.T) {
	require.Contains(t, channelMonitorV2HistogramSQL, "0::bigint")
	require.NotContains(t, channelMonitorV2HistogramSQL, "ul.user_id")
	require.Contains(t, channelMonitorV2HistogramRollupSQL, "h.user_id = 0")
	require.NotContains(t, channelMonitorV2ErrorAggregationSQL, "channel_monitor_v2_user_metrics_1m")
	for _, rule := range channelMonitorV2RetentionRules {
		require.NotContains(t, rule.table, "user_metrics")
	}
}
