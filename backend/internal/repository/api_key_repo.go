...
func (r *apiKeyRepository) ordinaryQuery() *dbent.APIKeyQuery {
	return r.activeQuery().Where(apikey.PurposeEQ(""))
}
...
func (r *apiKeyRepository) apiKeyListByUserIDQuery(userID int64, filters service.APIKeyListFilters) *dbent.APIKeyQuery {
	q := r.ordinaryQuery().Where(apikey.UserIDEQ(userID))
...
func (r *apiKeyRepository) VerifyOwnership(ctx context.Context, userID int64, apiKeyIDs []int64) ([]int64, error) {
	...
	ids, err := r.client.APIKey.Query().
		Where(apikey.UserIDEQ(userID), apikey.IDIn(apiKeyIDs...), apikey.DeletedAtIsNil(), apikey.PurposeEQ("")).
		IDs(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *apiKeyRepository) CountByUserID(ctx context.Context, userID int64) (int64, error) {
	count, err := r.ordinaryQuery().Where(apikey.UserIDEQ(userID)).Count(ctx)
...
func (r *apiKeyRepository) ListByGroupID(ctx context.Context, groupID int64, params pagination.PaginationParams) ([]service.APIKey, *pagination.PaginationResult, error) {
	q := r.ordinaryQuery().Where(apikey.GroupIDEQ(groupID))
...
func (r *apiKeyRepository) CountByGroupID(ctx context.Context, groupID int64) (int64, error) {
	count, err := r.ordinaryQuery().Where(apikey.GroupIDEQ(groupID)).Count(ctx)
	return int64(count), err
}
