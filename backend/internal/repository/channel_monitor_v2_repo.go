...
func (r *channelMonitorV2Repository) loadHistograms(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config, byBucket bool) ([]channelMonitorV2Histogram, error) {
	...
	group := "h.platform,h.group_id,h.model,h.metric,h.upper_bound_ms"
	if byBucket {
		if bucketSeconds > 0 {
	...
	rows, err := r.db.QueryContext(ctx, `SELECT `+bucketExpr+`,h.platform,h.group_id,h.model,h.metric,h.upper_bound_ms,SUM(h.sample_count) FROM `+channelMonitorV2HistogramTable(filter)+` h `+where+` GROUP BY `+group, args...)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&bucket, &h.Platform, &h.GroupID, &h.Model, &h.Metric, &h.UpperBound, &h.Count); err != nil {
			return nil, err
		}
