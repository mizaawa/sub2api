...
func CORS(cfg config.CORSConfig) gin.HandlerFunc {
	...
	allowHeaders := []string{
		...
		"accept", "origin", "Cache-Control", "X-Requested-With", "X-API-Key", "X-Sub2API-User-Email", "X-Admin-UI-Request", "X-User-UI-Request",
	}
	...
	return func(c *gin.Context) {
		...
		if originAllowed {
			...
			c.Writer.Header().Set("Access-Control-Expose-Headers", "ETag, Server-Timing, Content-Disposition, Content-Type, Content-Length")
