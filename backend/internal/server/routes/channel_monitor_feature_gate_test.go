...
func TestChannelMonitorV2RouteInventoryExcludesUserRanking(t *testing.T) {
	gin.SetMode(gin.TestMode)
	settings := newChannelMonitorModeSettings(true, service.ChannelMonitorModeV2)
	h := &handler.Handlers{
		ChannelMonitorV2: handler.NewChannelMonitorV2Handler(service.NewChannelMonitorV2Service(nil), nil),
	}

	userRouter := gin.New()
	registerChannelMonitorV2UserRoutes(userRouter.Group("/api/v1"), h, settings, nil)
	userRoutes := userRouter.Routes()
	require.Len(t, userRoutes, 5)
	for _, route := range userRoutes {
		require.NotEqual(t, "/api/v1/channel-monitor-v2/users", route.Path)
	}

	adminRouter := gin.New()
	registerChannelMonitorV2Routes(adminRouter.Group("/api/v1/admin"), h, settings)
	adminRoutes := adminRouter.Routes()
	require.Len(t, adminRoutes, 7)
	for _, route := range adminRoutes {
		require.NotEqual(t, "/api/v1/admin/channel-monitor-v2/users", route.Path)
	}
}
