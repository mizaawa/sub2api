-- Allow Custom as an OpenAI-compatible active-monitor provider and expose it
-- in passive-monitor V2 configuration without overwriting operator settings.
...
SET
    platforms = platforms || '[{"platform":"custom","enabled":true,"models":[]}]'::jsonb,
    version = version + 1,
    updated_at = NOW()
WHERE id = 1
  AND NOT EXISTS (
      SELECT 1
      FROM jsonb_array_elements(platforms) AS platform_config(value)
      WHERE platform_config.value->>'platform' = 'custom'
  );
