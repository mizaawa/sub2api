                ...
                <div class="overflow-x-auto">
                  <table class="min-w-full text-sm">
                    ...
                    <tbody class="space-y-2">
                      <tr v-for="p in (['anthropic', 'openai', 'gemini', 'antigravity', 'grok', 'custom'] as const)" :key="p" class="align-top">
                        <td class="pr-4 py-1">
                    ...
                    <div class="border-t border-gray-100 pt-4 dark:border-dark-700">
                      ...
                      <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                          ...
                          <tbody>
                            <tr v-for="p in (['anthropic', 'openai', 'gemini', 'antigravity', 'grok', 'custom'] as const)" :key="`${authSource.source}-pq-${p}`" class="align-top">
                              <td class="pr-4 py-1">
                                <span class="font-mono text-xs text-gray-700 dark:text-gray-300">{{ p }}</span>
                              </td>
                              <td class="pr-4 py-1">
          ...
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ localText('开启后，返回给下游的响应模型会使用下游请求的模型；上游映射和实际调用模型保持不变。', 'When enabled, client-facing response model fields use the downstream requested model while upstream routing remains unchanged.') }}
                </p>
              </div>
                    ...
                    <button
                      ...
                      :class="[
                        ...
                        isPaymentTypeEnabled(pt.value)
                          ? 'border-primary-500 bg-primary-500 text-primary-950 shadow-sm'
...
const paymentGuideHref = computed(() =>
  locale.value.startsWith("zh")
    ...
    : "https://github.com/mizaawa/sub2api/blob/main/docs/PAYMENT.md",
);

const paymentMethodsHref = computed(() =>
  locale.value.startsWith("zh")
    ? "https://github.com/mizaawa/sub2api/blob/main/docs/PAYMENT_CN.md#支持的支付方式"
    : "https://github.com/mizaawa/sub2api/blob/main/docs/PAYMENT.md#supported-payment-methods",
);
