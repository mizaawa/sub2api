...
describe('Image playground launcher', () => {
  it('opens the standalone project instead of embedding a local workbench', () => {
    ...
    expect(componentSource).toContain('STANDALONE_CONFIG_FALLBACK_STORAGE_KEY')
    expect(componentSource).toContain('localStorage.setItem(STANDALONE_CONFIG_FALLBACK_STORAGE_KEY')
    ...
    expect(componentSource).toContain('normalizeApiKeyRecord')
    expect(componentSource).toContain('currentAuthToken')
  ...
  it('retains the managed workbench capabilities while keeping removed surfaces out', () => {
    ...
    expect(standaloneScript).not.toContain('["api","API 配置"]')
    expect(standaloneScript).not.toContain('当前配置（API Key 名称）')
    expect(standaloneScript).not.toContain('aria-label":"打开设置"')
    expect(standaloneScript).not.toContain('aria-label":P?"收起输入框":"展开输入框"')
    expect(standaloneScript).not.toContain('children:"＋"')
    expect(standaloneScript).toContain('data-image-settings-dialog')
    expect(standaloneScript).toContain('data-image-prompt-row')
  ...
  it('binds the standalone cache to user identity without treating token rotation as logout', () => {
    ...
    expect(sessionComponent).toContain('x=v.userId??g?.userId')
    expect(sessionComponent).toContain('sessionValid=')
    expect(sessionComponent).toContain('localUserPresent=')
    expect(sessionComponent).toContain('bootstrapMatches=')
    expect(sessionComponent).toContain('ownerMismatch=')
    expect(sessionComponent).toContain('R=!!(!g&&x&&y&&v.tokenPresent&&')
    expect(sessionComponent).toContain('ownerMatches=')
    expect(sessionComponent).toContain('sessionValid=!identityMismatch&&S.tokenPresent')
    expect(sessionComponent).toContain('D=O||ownerMismatch?zs().then(()=>_):Promise.resolve(!0)')
    expect(sessionComponent).toContain('.then(H=>{if(!H)return;nr(!0);const ee=z.getState()')
  ...
  it('keeps a failed bootstrap available for a later standalone mount', () => {
    ...
    expect(standaloneScript).toContain('window.localStorage.getItem(zx+":fallback")')
    expect(standaloneScript).toContain('window.localStorage.removeItem(zx+":fallback")')
    expect(standaloneScript).toContain('Date.now()-p>120000')
  ...
  it('passes the selected Sub2API async profile to the standalone project', () => {
    ...
    expect(componentSource).toContain('responseFormatB64Json: true')
    ...
    expect(componentSource).toContain("'X-API-Key': key.key")
    ...
    expect(standaloneScript).toContain('responseFormatB64Json:s.responseFormatB64Json===true')
    ...
    expect(standaloneScript).toContain('x=z(j=>j.settings),j=z(j=>j.setSettings),S=z(j=>j.showToast)')
  ...
  it('uses the synchronous zayu image endpoints and keeps mobile controls inside the viewport', () => {
    ...
    expect(standaloneScript).toContain('data-image-profile-model')
    expect(standaloneScript).toContain('data-image-generate')
    expect(responsiveOverride).toContain('@media (max-width: 767px)')
