...
describe('UserPlatformQuotaModal', () => {
  ...
  it('已有数据正确填充 limit input', async () => {
    ...
    // 最后三个 input 是 custom 的 daily/weekly/monthly，默认无限额
    expect(inputs.slice(-3).map((input) => (input.element as HTMLInputElement).value)).toEqual(['', '', ''])
  })
  ...
  it('保存提交完整 6 platform payload', async () => {
    ...
    const custom = payload.find((p: any) => p.platform === 'custom')
